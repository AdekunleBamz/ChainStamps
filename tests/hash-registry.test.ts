...
describe("hash-registry", () => {
  ...
  it("should return correct hash fee", () => {
    const { result } = simnet.callReadOnlyFn("hash-registry", "get-hash-fee", [], wallet1);
    expect(result).toBeUint(HASH_FEE);
  });

  it("should start with zero hashes", () => {
    const { result } = simnet.callReadOnlyFn("hash-registry", "get-hash-count", [], wallet1);
  ...
  it("should allow user to store a hash", () => {
    ...
    const { result: countResult } = simnet.callReadOnlyFn("hash-registry", "get-hash-count", [], wallet1);
  ...
  it("should track fees collected", () => {
    ...
    const { result } = simnet.callReadOnlyFn("hash-registry", "get-total-fees", [], wallet1);
    expect(result).toBeUint(HASH_FEE);
  });
  ...
  it("should return contract owner", () => {
    const { result } = simnet.callReadOnlyFn("hash-registry", "get-contract-owner", [], wallet1);
  ...
  it("should retrieve hash by ID", () => {
    ...
    const { result } = simnet.callReadOnlyFn("hash-registry", "get-hash-by-id", [Cl.uint(1)], wallet1);
  ...
  it("should get hash info by id", () => {
    ...
    simnet.callPublicFn(
      "hash-registry",
      "store-hash",
      [Cl.buffer(testHash), Cl.stringUtf8("Info by id")],
      wallet1
    );
