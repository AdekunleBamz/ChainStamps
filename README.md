...
1. **Wallet**: Install either [Hiro Wallet](https://www.hiro.so/wallet) or [Xverse](https://www.xverse.app/).
2. **Network**: Switch to `Mainnet` for live transactions or `Devnet` for local testing.
3. **STX**: Ensure you have a small amount of STX for transaction fees (see the [Protocol Fees](#-protocol-fees) table).
